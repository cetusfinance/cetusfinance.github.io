...
Let's say we want to setup some curves to price USD/ZAR FX trades - that will require a setup something like this:

![CurveSetup](https://cetus.io/images/jacobianjuggling/CurveSetup.png)

The curve we need for pricing (USD/ZAR basis) we can fit to fx forwards and float-float cross-currency swaps.  These in turn will depend on USD LIBOR and ZAR JIBAR indices for the two floating legs - and forecasts for those indices will be fitted to vanilla IR fixed-float swaps.  Those vanilla swaps rely on OIS discount curves for their pricing which in turn can be fitted to float-float OIS-vs-IBOR basis swaps. 

So we need to solve five curves - two for each currency separately (forecast and OIS discounting) and one for the cross-currency basis. Now let's say each curve contains 20 benchmark instruments - this leads to 100 benchmarks in total. Computing a 100x100 Jacobian matrix could be slow, definitely not optimal.  Thankfully we can avoid doing that - we can solve the USD and ZAR problems first (and separately, possibly concurrently) which are both 40x40 matrices. We can follow this by using the output of the first stage (the two pairs of IBOR forecast and OIS discounting curves) to solve for the cross currency curve as a final step (merely a 20x20 matrix there). Not only is it much quicker due to removing unnecessary Jacobian calculations (10k vs. 3.6k elements per pass) but solving in stages also should improve convergence speed for the cases where a curve depends on another which can be pre-solved.
